<script>
    $(document).ready(function() {
        ...
        $.ajax({
			type: "GET",
			url: "<?= base_url('subkriteria/generate_kode/') ?>",
			success: function(data){
				$("#subkriteria_kode").val(data);
			}
        });

        $("#subkriteria_keterangan").select2({
        ...
        var table = $("#dtTable").dataTable({
            ...
            serverSide: true,
            ajax: {
                "url": "<?= base_url('subkriteria/list_data') ?>", 
                "type": "POST"
            },
            responsive: true,
            pageLength: 5,
            columns: [
                ...
                {"data": "subkriteria_kode"},
                {"data": "kriteria_nama"},
                {"data": "subkriteria_nama"},
                {"data": "subkriteria_keterangan"},
                {"data": "view"}
            ],
            order: [[0, 'asc']],
        ...
        $("#Frm").submit(function(e) {
            ...
            Pace.track(function() {
                jQuery.ajax({
                    type: "POST",
                    url: "<?= base_url('subkriteria/simpan/') ?>",
                    data: $("#Frm").serialize(),
        ...
        $(document).on('click', '#edit', function() {
            ...
            jQuery.ajax({
                type: "POST",
                url: "<?= base_url('subkriteria/get_data/') ?>",
                dataType: 'json',
                data: {
                    subkriteria_id: $(this).attr("data")
                },
                success: function(data) {
